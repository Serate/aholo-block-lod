...
function clusterWgsl(vecColumns: number) {
    return /* wgsl */ `
struct Uniforms {
    numPoints: u32,
    numCentroids: u32
...
interface GpuResource {
    ...
    gpuBuffers: {
        ...
        points: GPUBuffer[];
        centroids: GPUBuffer;
        centroidSq: GPUBuffer;
        result: GPUBuffer;
        resultReadBack: GPUBuffer;
    };
    backBuffers: {
        uniform: Uint32Array;
        points: Float32Array;
        centroids: Float32Array;
        centroidSq: Float32Array;
    };
    uploadedBatches: number[];
}

function packVec4Data(
    result: Float32Array,
    dataTable: Float32Array[],
    numRows: number,
    rowOffset: number,
    vecColumns: number,
    norms?: Float32Array,
...
export default class GpuClustering {
    ...
    private resource: GpuResource;
    private numBatches: number;
    private concurrencyBatches: number;
    private concurrencyRuns: number;
    private workgroupSize: number;
    private pointStride: number;
    private vecColumns: number;
    ...
    constructor(
        ...
        const workgroupsPerBatch = Math.max(
            1,
            Math.min(
                device.limits.maxComputeWorkgroupsPerDimension, // device dispatch limit
                Math.floor(storageLimit / (this.pointStride * this.workgroupSize * 4)), // point storage limit
                Math.ceil(numPoints / this.workgroupSize), // max limit
            ),
        );
        ...
        const gpuBuffers: GpuResource['gpuBuffers'] = {
        ...
        const backBuffers: GpuResource['backBuffers'] = {
        ...
        logger.info(
            `GPU k-means kernel bootstrapped with batch ${workgroupsPerBatch}*${this.workgroupSize}*${this.numBatches}, concurrency: ${this.concurrencyBatches}, runs: ${this.concurrencyRuns}`,
        );
    }

    async execute(points: Float32Array[], centroids: Float32Array[], labels: Uint32Array) {
        const {
            device,
            numPoints,
            numCentroids,
        ...
        packVec4Data(
            resource.backBuffers.centroids,
            centroids,
            numCentroids,
            0,
            vecColumns,
            resource.backBuffers.centroidSq,
        );
        ...
        for (let i = 0; i < concurrencyRuns; i++) {
            ...
            for (let j = 0; j < concurrencyBatches; j++) {
                ...
                if (resource.uploadedBatches[j] !== batchIndex) {
                    ...
                    device.queue.writeBuffer(
                        resource.gpuBuffers.points[j],
                        0,
                        resource.backBuffers.points.buffer,
                        0,
                        pointStride * currentBatchSize * 4,
                    );
                    resource.backBuffers.uniform[0] = currentBatchSize;
                    device.queue.writeBuffer(
                        resource.gpuBuffers.uniform,
                        256 * j,
                        resource.backBuffers.uniform.buffer,
                        0,
                        8,
                    );
            ...
            encoder.copyBufferToBuffer(
                resource.gpuBuffers.result,
                0,
                resource.gpuBuffers.resultReadBack,
                0,
                resultCount * 4,
            );
