...
const getModule = (function () {
    let m: DawnModule | undefined = undefined;
    return function () {
        if (!m) {
            m = createRequire(import.meta.url)(getNativePackageName() + '/dawn.node') as DawnModule;
...
export function initGPUAdapter(options: string[] = []) {
    if (!gpu) {
...
export async function createDevice() {
    ...
    logger.info(
        `WebGPU device created: ${device.adapterInfo.vendor}, ${device.adapterInfo.device}, ${device.adapterInfo.description}`,
    );

    device.addEventListener('uncapturederror', event => {
        const error = event.error as Error & { type?: string };
