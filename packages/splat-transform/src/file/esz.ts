...
import { ColIdx, type ISingleSplat, type SplatData } from '../SplatData.js';
...
export class EszFile implements IFile {
    ...
    async read(stream: ReadableStream<Uint8Array>, contentLength: number, data: SplatData) {
        ...
        const {
            min: [centerMinX, centerMinY, centerMinZ],
            max: [centerMaxX, centerMaxY, centerMaxZ],
        } = box;
    ...
    async write(stream: WritableStream<Uint8Array>, data: SplatData, indices: Uint32Array = mortonSort(data)) {
...

        {
            ...
            for (let i = 0; i < counts; i++) {
                ...
                const maxComp = TEMP_ROT.reduce(
                    (v, _, i) => (Math.abs(TEMP_ROT[i]) > Math.abs(TEMP_ROT[v]) ? i : v),
                    0,
                );
        ...
        if (shDegree > 0) {
            ...
            for (let i = 0; i < counts; i++) {
                ...
                for (let j = 0; j < shCoeffs; j++) {
                    ...
                    shN[(o + j) * 4 + 0] = clamp(
                        Math.floor(
                            (Math.round(table[ColIdx.shOffset + (j * 3 + 0)][idx] * 128) + 128 + scale / 2) / scale,
                        ) * scale,
                        0,
                        255,
                    );
                    shN[(o + j) * 4 + 1] = clamp(
                        Math.floor(
                            (Math.round(table[ColIdx.shOffset + (j * 3 + 1)][idx] * 128) + 128 + scale / 2) / scale,
                        ) * scale,
                        0,
                        255,
                    );
                    shN[(o + j) * 4 + 2] = clamp(
                        Math.floor(
                            (Math.round(table[ColIdx.shOffset + (j * 3 + 2)][idx] * 128) + 128 + scale / 2) / scale,
                        ) * scale,
                        0,
                        255,
                    );
